...
function updateWindowChildrenQuery(pushNewUrl) {
    ...
    if (windowUrl.children!= null && windowUrl.children.length == 1 && windowUrl.getChildrenItem(tanFieldsName.mainBlockName) != null)
    {
        _tmpUrl = windowUrl.getChildrenItem(tanFieldsName.mainBlockName);
        if (_tmpUrl.children == null || _tmpUrl.children.length == 0)
            windowUrl.children = null;
    }
...
function webHistoryPopState(event) {
    ...
    var childrenCount = (UrlOfWindows.children == null || UrlOfWindows.children.length == 0) ?
        (  ($("." + tanFieldsName.mainBlockName).length !=0 )? 1:0)
        : UrlOfWindows.children.length;
    if ( childrenCount < changableBlocksList.length) {
    ...
    changableBlocksList.each(function () {     // agar page bedune layout bashe, subpartesh in kelas ro nadare. pas bayad selector ro avaz kard va bara un mavared, adreshesh ro az children.children peyda kard
        let tanBlock = $(this);
        let expectedUrl;
        if (tanBlock.attr("id") == tanFieldsName.mainBlockName) {
            var blockOldUrl = new tanUrl(tanBlock.attr("url"));
            if (UrlOfWindows.children != null && UrlOfWindows.getChildrenItem(tanFieldsName.mainBlockName) != null) 
                expectedUrl = UrlOfWindows.getChildrenItem(tanBlock.attr("id"));
            else {
                expectedUrl = new tanUrl(UrlOfWindows.toString());
                expectedUrl.children = null;
            }
        }
        else 
            expectedUrl = UrlOfWindows.getChildrenItem(tanBlock.attr("id"));
...
(function ($) {
    ...
    $.fn.Ajax_LoadURL = function (url, method, data, options, preBlockName, influenceParentsAndWindowsUrl, callbackfunction) {
        ...
        if (typeof callbackfunction != "undefined" && callbackfunction != null
            & (url._schemaAndDomainAuthorityAndLocalUrl == ""
               || obj_response.CloseLayout == TanLayoutCloseState.CloseAndRefereshDependentBlocks
               || obj_response.CloseLayout == TanLayoutCloseState.CloseWidthoutAnyReferesh)
            ) {
    ...
    $.fn.tempUpdateBlocklinksAndFormsAfterContentChange = function ()
    {
        var tanBlock = $(this);
